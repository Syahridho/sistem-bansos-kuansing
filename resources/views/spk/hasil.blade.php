<x-app-layout>
    ...
    <div class="rounded-lg bg-zinc-900 px-5 py-4 mb-5 flex items-center justify-between gap-4">
        <div class="space-y-1">
            ...
            <p class="text-xs text-zinc-500 flex items-center gap-1.5">
                ...
                {{ $periode->tanggal_mulai->format('d-m-Y') }} – {{ $periode->tanggal_akhir->format('d-m-Y') }}
            </p>
        </div>
    ...
    @if($hasilAkhir->isNotEmpty())
        ...
        {{-- Status Penetapan Banner --}}
        <div class="rounded-xl border p-4 sm:p-5 shadow-sm mb-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 transition-all
            {{ $periode->status === 'tutup' ? 'bg-zinc-50 border-zinc-200' : 'bg-blue-50/40 border-blue-100' }}">
            <div class="flex items-start gap-3">
                @if($periode->status === 'tutup')
                    <div class="p-2.5 bg-zinc-900 text-white rounded-lg shrink-0">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                        </svg>
                    </div>
                    <div>
                        <h4 class="text-sm font-bold text-zinc-900">Hasil Perangkingan Telah Dikunci</h4>
                        <p class="text-xs text-zinc-500 mt-0.5">Periode ini telah ditutup secara resmi. Data alternatif dan hasil keputusan tidak dapat diubah kembali.</p>
                    </div>
                @else
                    <div class="p-2.5 bg-blue-600 text-white rounded-lg shrink-0">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-blue-900">Penetapan Penerima Bantuan</h4>
                        <p class="text-xs text-blue-700/80 mt-0.5">Lakukan penetapan hasil untuk mengunci data alternatif warga dan menutup periode penyaluran bantuan ini secara resmi.</p>
                    </div>
                @endif
            </div>
            ...
            <div class="shrink-0">
                @if($periode->status === 'tutup')
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-zinc-900 text-zinc-100 shadow-sm border border-zinc-950">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" />
                        </svg>
                        Telah Ditetapkan & Terkunci
                    </span>
                @else
                    @if(auth()->user()->role === 'admin')
                        <form action="{{ route('periode.toggle-status', $periode) }}" method="POST"
                              @submit.prevent="triggerConfirm($event.target, 'Tetapkan Penerima & Kunci Hasil', 'Apakah Anda yakin ingin menetapkan penerima bantuan dan menutup periode ini secara resmi?', 'Tindakan ini akan mengunci seluruh data alternatif warga dan hasil penilaian di dalamnya secara permanen agar tidak dapat diubah kembali.', 'Ya, Tetapkan & Kunci')">
                            @csrf @method('PATCH')
                            <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold shadow-sm transition">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z" />
                                </svg>
                                Tetapkan Penerima
