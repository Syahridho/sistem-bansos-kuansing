            ...
            <form action="{{ route('kriteria.store') }}" method="POST" x-data="kriteriaForm()">
                ...
                <div class="space-y-5">
                    <div class="mb-5">
                        ...
                        <select id="assistance_type_id" required class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 {{ $selectedTypeId ? 'bg-gray-50 cursor-not-allowed' : '' }}" {{ $selectedTypeId ? 'disabled' : 'name=assistance_type_id' }}>
                        ...
                        @if($selectedTypeId)
                            <input type="hidden" name="assistance_type_id" value="{{ $selectedTypeId }}">
                        @endif
                    </div>
